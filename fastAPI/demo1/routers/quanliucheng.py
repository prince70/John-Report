...
# report_temp 使用独立数据表；默认指向全流程工序表。
REPORT_TEMP_TABLE = os.getenv("REPORT_TEMP_TABLE", "全流程工序")
TABLE_SQL = f"[{DB_DATABASE}].[dbo].[{REPORT_TEMP_TABLE.replace(']', ']]')}]"
...
def get_process_id_columns(db_columns):
    # report_temp 使用“工序ID筛选”，优先识别工序ID相关列。
    candidates = ["工序ID", "工序Id", "工序id", "工序", "OpExternalId", "OpId"]
    exact = [col for col in db_columns if str(col) in candidates]
    if exact:
        return exact
    ...
    for col in db_columns:
        name = str(col)
        if re.search(r"工序", name) and not re.search(r"报工数|结束时间|最早结束时间|最晚结束时间|时间|日期", name):
            fuzzy.append(col)
    return fuzzy


def is_hidden_output_column(col_name: str) -> bool:
    return str(col_name or "").strip().lower() == "column1"


def get_employee_search_columns(db_columns):
    # 优先匹配常见员工字段；不存在时再做模糊匹配。
    preferred = ["姓名", "员工姓名", "员工名称", "emp_name", "工号", "emp_item_no", "员工编号", "emp_no"]
    exact = [col for col in db_columns if str(col).strip() in preferred]
    if exact:
        return exact
    ...
    for col in db_columns:
        ...
        if re.search(r"姓名|员工|工号|emp", name):
            fuzzy.append(col)
    return fuzzy
...
def build_report_where(
    ...
    if product_spec:
        # report_temp 页面将“产品规格”输入框复用为“员工查询”。
        keyword = str(product_spec).strip()
        employee_cols = get_employee_search_columns(db_columns)
        if employee_cols:
            employee_ors = []
            for col in employee_cols:
                col_sql = escape_sql_identifier(col)
                employee_ors.append(f"LTRIM(RTRIM(CONVERT(NVARCHAR(255), {col_sql}))) LIKE ?")
                params.append(f"%{keyword}%")
            conditions.append("(" + " OR ".join(employee_ors) + ")")
        elif "产品规格" in db_columns:
            # 回退：若仍是旧结构，则按产品规格筛选。
    ...
    process_id_columns = get_process_id_columns(db_columns)
    if selected_processes and process_id_columns:
        placeholders = ", ".join(["?"] * len(selected_processes))
        process_ors = []
        for col in process_id_columns:
...
def build_report_query(
    ...
    # report_temp 默认按“开始时间”升序；无该列时回退到原有规则。
    start_time_col = next(
        (col for col in db_columns if re.search(r"开始时间|StartDate", str(col))),
        None,
    )
    ...
    sort_col = start_time_col if start_time_col else (first_process_time_col if first_process_time_col else (db_columns[0] if db_columns else "料品编码"))
...
def fetch_report_total(
    ...
    count_query = "SELECT COUNT(1) FROM " + TABLE_SQL + where_sql
...
def fetch_report_rows(
    ...
    visible_indexes = [i for i, col in enumerate(original_columns) if not is_hidden_output_column(col)]
    columns = [original_columns[i] for i in visible_indexes]
    ...
    for row in rows:
        ...
        for i in visible_indexes:
            ...
            value = normalize_datetime_placeholder(col_name, value)
            if hasattr(value, "isoformat"):
                row_dict[col_name] = value.isoformat()
            else:
                row_dict[col_name] = "-" if value is None else value
        data.append(row_dict)

    # report_temp 为全流程工序页面，返回原始列结构，不做首尾工序重命名。
...
def add_missing_columns(cursor, headers, db_columns):
    ...
    for col in missing_columns:
        ...
        cursor.execute(
            f"ALTER TABLE {TABLE_SQL} ADD {escape_sql_identifier(col)} NVARCHAR(MAX) NULL"
        )
...
def drop_extra_columns(cursor, headers, db_columns):
    ...
    for col in extra_columns:
        cursor.execute(
            f"ALTER TABLE {TABLE_SQL} DROP COLUMN {escape_sql_identifier(col)}"
        )
...
async def get_report_raw_data(
    ...
    try:
        ...
        # 直接查询，不进行任何转换
        query = f"SELECT TOP {limit} * FROM {TABLE_SQL}"
...
async def get_report_data(
    ...
    try:
        ...
        return {
            ...
            "columns": columns,
            "total": total,
            "source_table": REPORT_TEMP_TABLE,
            "timestamp": datetime.now().isoformat(),
        }
    except HTTPException:
        raise
...
async def get_report_filter_options(
    ...
    try:
        ...
        db_columns = get_table_columns(cursor)
        process_columns = get_process_id_columns(db_columns)
        date_columns = [
            col
            for col in db_columns
            if (
...
async def import_report_excel(file: UploadFile = File(...)):
    ...
    try:
        ...
        insert_sql = (
            f"INSERT INTO {TABLE_SQL} ({', '.join(escaped_columns)}) "
            f"VALUES ({placeholders})"
        )

        # 清空数据表
        cursor.execute("DELETE FROM " + TABLE_SQL)
        ...
        # 导入成功后回查数据库行数
        cursor.execute("SELECT COUNT(1) FROM " + TABLE_SQL)
