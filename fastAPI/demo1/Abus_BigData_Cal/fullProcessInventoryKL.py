...
def get_data_from_db(订单批号=None, 料品编码=None, 料品名称=None, 料品规格=None, 当前工序ID=None, 下一道工序ID=None, 开始日期=None, 结束日期=None):
    ...
    try:
        ...
        if 结束日期:
            sql += " AND [FinishedDate] < ?"
            params.append((datetime.strptime(结束日期, "%Y-%m-%d") + timedelta(days=1)).strftime("%Y-%m-%d"))
        sql += " ORDER BY [id]"
...
async def get_data(
    ...
    下一道工序ID: Optional[str] = Query(None), 开始日期: Optional[str] = Query(None),
    结束日期: Optional[str] = Query(None),
):
    try:
