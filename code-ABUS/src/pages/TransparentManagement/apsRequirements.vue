<!-- 2025-9-29新增审核需求APS Requirements 项目申请 -->
<template>
  <Layout :breadcrumbItems="breadcrumbItems">
    ...
    <!-- 新建项目对话框 -->
    <el-dialog title="新建项目需求" :visible.sync="showDialog" width="600px">
...
export default {
  ...
  data() {
    return {
      ...
      // 面包屑导航路径，显示在页面顶部
      breadcrumbItems: ['透明化管理', 'APS 项目需求'],
      ...
      // 状态下拉选项配置数组，包含三种审核状态
      statusOptions: [
        { label: '待审批', value: '待审批' },  // 蓝色矩形按钮
        { label: '同意', value: '同意' },      // 绿色矩形按钮
        { label: '拒绝', value: '拒绝' }       // 红色矩形按钮
      ],
  ...
  methods: {
    ...
    async fetchData(bust = false) {
      ...
      try {
        ...
        const res = await axios.get('/api/apsRequirements', { params })
        if (res.data?.status === 'success') {
          const rows = (res.data.data || []).map(item => {
            const processed = {}
            Object.entries(item).forEach(([key, val]) => {
              if (this.dateFields.includes(key)) {
                processed[key] = typeof val === 'string' ? val : ''
              } else {
                processed[key] = val
              }
            })
          ...
          if (rows.length > 0) {
            this.allColumns = Object.keys(rows[0]).map(k => ({
              prop: k,
              label: k,
              width: k === '申请时间' ? 160 : 180,
              sortable: this.dateFields.includes(k)
            }))
    ...
    async submitForm() {
      try {
        ...
        const res = await axios.post('/api/apsRequirements', this.form)
        if (res.data.status === 'success') {
    ...
    async saveDateEdit() {
      ...
      try {
        const res = await axios.put(
          `/api/apsRequirements/${this.editingRow.RequestID}/approval`,
    ...
    async handleStatusChange(row, newVal) {
      ...
      try {
        await this.$confirm(`确认将 RequestID 为 ${row.RequestID} 的项目状态修改为 “${newVal}” 吗？`, '确认修改', {
      ...
      try {
        const res = await axios.put(`/api/apsRequirements/${row.RequestID}/status`, { 状态: newVal }, { params: { username: this.currentUsername } })
        if (res.data?.status === 'success') {
