  ...
  computed: {
    ...
    filteredTableData() {
      ...
      if (this.selectedDateColumn && this.selectedDateRange.length === 2) {
        const hasSelectedDateColumn = rows.some((row) =>
          row && Object.prototype.hasOwnProperty.call(row, this.selectedDateColumn)
        )
        if (!hasSelectedDateColumn) {
          return rows
        }
  ...
  methods: {
    ...
    async loadWorkshopOptions(force = false) {
      ...
      try {
        ...
        const { data } = await axios.get('/api/report/workshops', { params })
    ...
    async fetchReportBatch(params = {}, offset = 0, limit = 2000) {
      ...
      const { data } = await axios.get('/api/report', { params: reqParams })
      if (data?.status !== 'success') {
    ...
    async loadProcessDateOptions(force = false) {
      ...
      try {
        ...
        const { data } = await axios.get('/api/report/filter-options', { params })
        if (data?.status !== 'success') {
        ...
        if (!dateColumns.includes(this.selectedDateColumn)) {
          // 优先自动选中第1道工序时间列，否则取第一个可用日期列
          const firstProcessDate = dateColumns.find((col) => /第\s*1\s*道工序/.test(col))
          this.selectedDateColumn = firstProcessDate || dateColumns[0] || ''
          if (!this.selectedDateColumn) {
            this.selectedDateRange = []
          }
        }
    ...
    async handleImportRequest({ file }) {
      ...
      try {
        ...
        const { data } = await axios.post('/api/report/import', formData, {
          headers: {
    ...
    updateColumnWidthMap() {
      ...
      this.displayTableColumns.forEach((col) => {
        const isEarlyLateTimeCol = /最早结束时间|最晚结束时间/.test(String(col || ''))
        // 表头允许换行显示，避免长标题把列宽撑得过大。
        let maxLen = Math.min(String(col || '').length, isEarlyLateTimeCol ? 10 : 12)
        ...
        const baseWidth = isEarlyLateTimeCol ? (maxLen * 12 + 18) : (maxLen * 16 + 32)
        const minWidth = isEarlyLateTimeCol ? 88 : 120
        const maxWidth = isEarlyLateTimeCol ? 160 : 420
        widthMap[col] = Math.max(minWidth, Math.min(baseWidth, maxWidth))
      })
    ...
    async onProcessColumnDropdownVisible(colName, visible) {
      ...
      try {
        ...
        const { data } = await axios.get('/api/report/column-options', { params })
        if (data?.status !== 'success') {
