      ...
      <div class="el-card is-always-shadow mb-4 filter-card">
        <div class="el-card__body">
          <el-form @submit.native.prevent="searchReport(true)" label-width="90px">
            ...
            <div class="query-tip">页面已自动加载，可调整条件后点击查询</div>
  ...
  computed: {
    ...
    workshopFilteredData() {
      ...
      return this.tableData.filter((row) => {
        const tokens = this.normalizeWorkshopValues(row?.[this.workshopColumn])
        return tokens.some((token) => this.selectedWorkshops.includes(token))
      })
    },
    filteredTableData() {
  ...
  methods: {
    ...
    async loadWorkshopOptions(force = false) {
      ...
      try {
        ...
        const { data } = await axios.get('/api/report_temp/report/workshops', { params })
        ...
        const normalized = Array.from(
          new Set(
            options.flatMap((item) => this.normalizeWorkshopValues(item))
          )
    ...
    async fetchReportBatch(params = {}, offset = 0, limit = 2000) {
      ...
      const { data } = await axios.get('/api/report_temp/report', { params: reqParams })
      if (data?.status !== 'success') {
    ...
    async loadProcessDateOptions(force = false) {
      ...
      try {
        ...
        const { data } = await axios.get('/api/report_temp/report/filter-options', { params })
        if (data?.status !== 'success') {
        ...
        if (!dateColumns.includes(this.selectedDateColumn)) {
          // report_temp 优先自动选中“开始时间”列，否则回退第一个可用日期列。
          const startDateCol = dateColumns.find((col) => /开始时间/.test(String(col || '')))
          this.selectedDateColumn = startDateCol || dateColumns[0] || ''
          if (!this.selectedDateColumn) {
            this.selectedDateRange = []
          }
        }
    ...
    async exportExcel() {
      ...
      try {
        ...
        const response = await axios.get('/api/report_temp/report/export', {
          params,
          responseType: 'blob'
        })
    ...
    async handleImportRequest({ file }) {
      ...
      try {
        ...
        const { data } = await axios.post('/api/report_temp/report/import', formData, {
          headers: {
    ...
    updateWorkshopOptions() {
      ...
      this.tableData.forEach((row) => {
        this.normalizeWorkshopValues(row?.[this.workshopColumn]).forEach((item) => {
          set.add(item)
        })
      })
    ...
    async onProcessColumnDropdownVisible(colName, visible) {
      ...
      try {
        ...
        const { data } = await axios.get('/api/report_temp/report/column-options', { params })
        if (data?.status !== 'success') {
    ...
    updateDateColumnOptions() {
      ...
      if (!options.includes(this.selectedDateColumn)) {
        const startDateCol = options.find((col) => /开始时间/.test(String(col || '')))
        this.selectedDateColumn = startDateCol || options[0] || ''
        if (!this.selectedDateColumn) {
          this.selectedDateRange = []
        }
      }
