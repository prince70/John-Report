...
data() {
  return {
    ...
    productAttributes: [], // 锁类分区选项
    tableData: [],
    loading: false,
    lastWeekDiff: null, // 新增：存储上周差值
    lastWeekNumber: null // 新增：存储上周的周数
  }
},
async mounted() {
  ...
  // 动态获取锁类分区选项
  try {
    const response = await axios.get('/api/packagingDoneCal/areas');
    if (response.data?.status === 'success') {
      const areas = response.data.data || [];
      this.productAttributes = areas.map(area => ({
        value: area,
        label: area
      }));
...
methods: {
  ...
  async fetchData() {
    ...
    try {
      console.log('连接中！！！');
      const response = await axios.get('/api/packagingDoneCal');
      ...
      if (response.data?.status === 'success') {
        ...
        this.dynamicColumns = processedData.length
