...
export default {
  ...
  data() {
    return {
      breadcrumbItems: ['报表页面', '全流程报工库存-打磨车间-装配区'],
  ...
  methods: {
    generateBreadcrumb(path) {
      ...
      } catch { this.breadcrumbItems = ['报表页面', '全流程报工库存-打磨车间-装配区'] }
    },
    ...
    async loadOptions() {
      try {
        ...
        const results = await Promise.all(fields.map(f => axios.get('/api/fullProcessInventoryDM_ZPQ/options', { params: { field: f } }).then(res => ({ f, d: res.data })).catch(() => ({ f, d: { data: [] } }))))
    ...
    async searchData() {
      ...
      try {
        ...
        const [dataRes, summaryRes] = await Promise.all([
          axios.get('/api/fullProcessInventoryDM_ZPQ', { params }),
          axios.get('/api/fullProcessInventoryDM_ZPQ/summary', { params })
        ])
