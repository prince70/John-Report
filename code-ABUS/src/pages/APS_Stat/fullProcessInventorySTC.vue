...
export default {
  ...
  data() {
    return {
      breadcrumbItems: ['工序', '全流程报工库存', '锁体C车间'],
  ...
  methods: {
    generateBreadcrumb(path) {
      ...
      } catch { this.breadcrumbItems = ['工序', '全流程报工库存', '锁体C车间'] }
    },
    ...
    async searchData() {
      ...
      try {
        ...
        const [dataRes, summaryRes] = await Promise.all([
          axios.get('/api/fullProcessInventorySTC', { params }),
          axios.get('/api/fullProcessInventorySTC/summary', { params })
        ])
