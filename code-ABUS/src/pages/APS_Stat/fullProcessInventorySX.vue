...
export default {
  ...
  data() {
    return {
      breadcrumbItems: ['报表页面', '全流程报工库存-锁配件车间'],
  ...
  methods: {
    generateBreadcrumb(path) {
      ...
      } catch { this.breadcrumbItems = ['报表页面', '全流程报工库存-锁配件车间'] }
    },
    ...
    async loadOptions() {
      try {
        ...
        const results = await Promise.all(fields.map(f => axios.get('/api/fullProcessInventorySX/options', { params: { field: f } }).then(res => ({ f, d: res.data })).catch(() => ({ f, d: { data: [] } }))))
    ...
    async searchData() {
      ...
      try {
        ...
        const [dataRes, summaryRes] = await Promise.all([
          axios.get('/api/fullProcessInventorySX', { params }),
          axios.get('/api/fullProcessInventorySX/summary', { params })
        ])
