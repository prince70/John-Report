...
export default {
  ...
  data() {
    return {
      breadcrumbItems: ['工序', '全流程报工库存', '锁体A车间'],
  ...
  methods: {
    generateBreadcrumb(path) {
      ...
      } catch { this.breadcrumbItems = ['工序', '全流程报工库存', '锁体A车间'] }
    },
    ...
    async searchData() {
      ...
      try {
        ...
        const [dataRes, summaryRes] = await Promise.all([
          axios.get('/api/fullProcessInventorySTA', { params }),
          axios.get('/api/fullProcessInventorySTA/summary', { params })
        ])
