      ...
      <div v-if="hasSearched" class="el-card is-always-shadow table-card" v-loading="loading" element-loading-text="加载中...">
        <div class="el-card__body">
          <el-table v-if="tableData.length" :data="tableData" border stripe max-height="620" style="width: 100%" :header-cell-style="{ background: '#eef1f6', color: '#606266' }">
            ...
            <el-table-column prop="emp_no" label="emp_no" min-width="100" show-overflow-tooltip />
            ...
            <el-table-column prop="车间提供" label="车间提供" min-width="100" align="right">
              <template #default="scope"><span>{{ scope.row.车间提供 !== null ? formatNumber(scope.row.车间提供) : '-' }}</span></template>
            </el-table-column>
            <el-table-column prop="时间" label="时间" min-width="100" align="right">
              <template #default="scope"><span>{{ scope.row.时间 !== null ? formatNumber(scope.row.时间) : '-' }}</span></template>
            </el-table-column>
          </el-table>
...
export default {
  ...
  data() {
    ...
    return {
      breadcrumbItems: ['报工', '车间报工详情', 'CNC报工详情'],
      filters: { start: fmt(yesterday), end: fmt(today), 订单批号: '' },
  ...
  methods: {
    generateBreadcrumb(path) {
      try {
        const find = (menus, p) => { for (const m of menus) { for (const s of (m.children || [])) { for (const gc of (s.children || [])) { for (const c of (gc.children || [])) { if (c.path === p) return [s.name, gc.name, c.name] } if (gc.path === p) return [s.name, gc.name] } } } const seg = p.split('/').pop(); return ['报工', '车间报工详情', 'CNC报工详情'] }
        const r = find(this.sidebarMenus, '/' + path.split('/').filter(p => p).join('/'))
        this.breadcrumbItems = Array.isArray(r) ? r : [r]
      } catch { this.breadcrumbItems = ['报工', '车间报工详情', 'CNC报工详情'] }
    },
    ...
    async searchData() {
      if (!this.filters.start || !this.filters.end) { this.$message.warning('请选择报工日期范围'); return }
      this.loading = true; this.hasSearched = true
      try {
        const params = this.getFilterParams()
        params.page = this.currentPage
        params.page_size = this.pageSize
        const res = await axios.get('/api/workshopReportDetail/CNC', { params })
    ...
    resetFilters() {
      ...
      const today = new Date()
      const fmt = d => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`
      this.filters = { start: fmt(yesterday), end: fmt(today), 订单批号: '' }
    ...
    async exportData() {
      ...
      try {
        const params = this.getFilterParams()
        const res = await axios.get('/api/workshopReportDetail/CNC/export', { params, responseType: 'blob' })
        const cd = res.headers['content-disposition']; let fname = 'CNC报工详情.xlsx'; if (cd) { const m = cd.match(/filename\*=UTF-8''(.+)/); if (m) fname = decodeURIComponent(m[1]); else { const p = cd.split('filename=')[1]; if (p) fname = p.replace(/"/g, '') } }
