  ...
  methods: {
    ...
    async exportData() {
      ...
      try {
        ...
        if (this.filters.生产线编号) params.生产线编号 = this.filters.生产线编号.trim()
        if (this.filters.工单批号) params.工单批号 = this.filters.工单批号.trim()
        const res = await axios.get('/api/workshopReportDetail/DamoDM/export', { params, responseType: 'blob' })
        const cd = res.headers['content-disposition']; let fname = '打磨区报工详情.xlsx'; if (cd) { const m = cd.match(/filename\*=UTF-8''(.+)/); if (m) fname = decodeURIComponent(m[1]); else { const p = cd.split('filename=')[1]; if (p) fname = p.replace(/"/g, '') } }
