...
export default {
  ...
  data() {
    return {
      breadcrumbItems: ['报表页面', '全流程报工库存-装嵌车间-胆仔锁区'],
  ...
  methods: {
    generateBreadcrumb(path) {
      ...
      } catch { this.breadcrumbItems = ['报表页面', '全流程报工库存-装嵌车间-胆仔锁区'] }
    },
    ...
    async loadOptions() {
      try {
        ...
        const results = await Promise.all(fields.map(f => axios.get('/api/fullProcessInventoryZQ_DZSQ/options', { params: { field: f } }).then(res => ({ f, d: res.data })).catch(() => ({ f, d: { data: [] } }))))
    ...
    async searchData() {
      ...
      try {
        ...
        const [dataRes, summaryRes] = await Promise.all([
          axios.get('/api/fullProcessInventoryZQ_DZSQ', { params }),
          axios.get('/api/fullProcessInventoryZQ_DZSQ/summary', { params })
        ])
