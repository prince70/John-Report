      ...
      <div v-if="hasSearched" class="el-card is-always-shadow table-card" v-loading="loading" element-loading-text="加载中...">
        <div class="el-card__body">
          <el-table v-if="tableData.length" :data="tableData" border stripe max-height="620" style="width:100%" :header-cell-style="{ background: '#eef1f6', color: '#606266' }">
            ...
            <el-table-column prop="报工重量" label="报工重量" min-width="100" align="right">
              <template #default="scope"><span>{{ formatNumber(scope.row.报工重量) }}</span></template>
            </el-table-column>
...
export default {
  ...
  data() {
    ...
    return {
      breadcrumbItems: ['报工', '车间报工详情', '锁梁报工详情'],
      filters: { start: fmt(yesterday), end: fmt(today), 序列号: '', 姓名: '', 订单批号: '' },
  ...
  methods: {
    generateBreadcrumb() { this.breadcrumbItems = ['报工', '车间报工详情', '锁梁报工详情'] },
    ...
    async searchData() {
      ...
      try {
        ...
        const res = await axios.get('/api/workshopReportDetail/Suoliang', { params })
    ...
    resetFilters() {
      ...
      const today = new Date()
      const fmt = d => `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`
      this.filters = { start: fmt(yesterday), end: fmt(today), 序列号: '', 姓名: '', 订单批号: '' }
    ...
    async exportData() {
      ...
      try {
        ...
        if (this.filters.姓名) params.姓名 = this.filters.姓名.trim()
        if (this.filters.订单批号) params.订单批号 = this.filters.订单批号.trim()
        const res = await axios.get('/api/workshopReportDetail/Suoliang/export', { params, responseType: 'blob' })
        const cd = res.headers['content-disposition']; let fname = '锁梁报工详情.xlsx'; if (cd) { const m = cd.match(/filename\*=UTF-8''(.+)/); if (m) fname = decodeURIComponent(m[1]); else { const p = cd.split('filename=')[1]; if (p) fname = p.replace(/"/g, '') } }
