  ...
  export default {
    ...
    data() {
      return {
        ...
        productAttributes:[
          { value: '1', label: '打磨车间-打磨区' },
          { value: '2', label: '功能锁区' },
          { value: '3', label: '铝门锁胆仔区' },
          { value: '4', label: '配件区' },
          { value: '5', label: '喷油区' }
        ],
        tableData: [],
    ...
    created() {
      eventBus.$on('sidebar-Menus-Updated', (menus) => {
        ...
        // console.log('获取到的侧边栏菜单22222:', this.sidebarMenus);
    ...
    methods: {
      generateBreadcrumb(path) {
        try{
          ...
          this.breadcrumbItems = ['落货需求数量统计工时'];
        }
      },
      async fetchData() {
        this.loading = true;
        try {
          console.log('连接中！！！');
          const response = await axios.get('/api/polishingNeedCal');
      ...
      navigateTo(path) {
        ...
        this.fetchData().then(() => {
          ...
          this.calculateWeeklyStats(); // 搜索后计算未来4周统计
        });
        if (!this.queryParams.productAttribute && 
            !this.queryParams.startTime && 
            !this.queryParams.endTime) {
          this.dynamicData = [...this.originalDataBackup];
          this.calculateWeeklyStats(); // 重置时计算未来4周统计
        }
      },
      resetForm() {
        this.queryParams = {
