        ...
        <div class="search-form">
            <el-form :model="queryParams" label-width="100px">
                ...
                <!-- 车间资讯 -->
                  ...
                  <el-row v-if="lastWeekDiff !== null" :gutter="20">
                    <el-col :span="8">
...
computed: {
    diffLabel() {
      if (this.lastWeekDiff === null || this.lastWeekNumber === null) {
        return '';
      }
...
methods: {
    generateBreadcrumb(path) {
        ...
        this.breadcrumbItems = ['锁体车间-报工数量统计工时'];
        }
    },
    async fetchData() {
    this.loading = true;
    try {
        console.log('连接中！！！');
        const response = await axios.get('/api/stWork');
    ...
    computeLastWeekDiff() {
      ...
      const filtered = this.originalDataBackup.filter(item => {
        if (selectedLabel && item['锁类分区'] !== selectedLabel) return false;
        return true;
      });
      if (!filtered.length) {
        this.lastWeekDiff = null;
      ...
      const targetWeek = weeks[1]; // 第二新的周
...
/* 车间资讯输入框背景保持白色 */
/* 去除禁用背景色 */
.no-bg /deep/ .el-input__inner {
  background-color: #ffffff !important;
}

/* 默认文本颜色 */
.diff-input /deep/ .el-input__inner {
  color: #303133;
}

/* 负值时文本变红 */
.diff-input.negative /deep/ .el-input__inner {
  color: red !important;
}
</style>