...
methods: {
  ...
  async fetchData() {
    ...
    try {
      console.log('连接中！！！');
      const response = await axios.get('/api/rawCuttingDoneCal');
  ...
  computeLastWeekDiff() {
    if (!Array.isArray(this.dynamicData) || this.dynamicData.length === 0) {
      this.lastWeekDiff = null;
      this.lastWeekNumber = null;
      return;
    }
    const weeks = Array.from(new Set(this.dynamicData.map(r=>r['周数']).filter(w=>typeof w==='number'))).sort((a,b)=>b-a);
    if (weeks.length < 2) {
      this.lastWeekDiff = null;
      this.lastWeekNumber = null;
      return;
    ...
    const row = this.dynamicData.find(r=>r['周数']===targetWeek);
    if (!row) {
      this.lastWeekDiff = null;
      return;
    }
