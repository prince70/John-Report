        ...
        <div class="el-card__body">
          <el-form @submit.native.prevent="searchData" label-width="100px">
            <el-row :gutter="20">
              ...
              <el-col :span="6">
                <el-form-item label="锁类分区">
                  <el-input v-model="filters.锁类分区" placeholder="请输入锁类分区" clearable @keyup.enter.native="searchData" />
                </el-form-item>
              </el-col>
              <el-col :span="6">
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="6">
                <el-form-item label="含TSA">
                  <el-checkbox v-model="filters.含TSA" />
                </el-form-item>
              </el-col>
            </el-row>
...
export default {
  ...
  data() {
    ...
    return {
      breadcrumbItems: ['报工', '车间报工详情', '装嵌报工详情'],
      filters: { start: fmt(yesterday), end: fmt(today), 锁类分区: '', 序列号: '', 姓名: '', 工单批号: '', 含TSA: true },
  ...
  methods: {
    generateBreadcrumb() { this.breadcrumbItems = ['报工', '车间报工详情', '装嵌报工详情'] },
    ...
    async searchData() {
      ...
      try {
        const params = { start: this.filters.start, end: this.filters.end, 含TSA: this.filters.含TSA, page: this.currentPage, page_size: this.pageSize }
        ...
        if (this.filters.锁类分区) params.锁类分区 = this.filters.锁类分区.trim()
        const res = await axios.get('/api/workshopReportDetail/Zhuangqian', { params })
    ...
    resetFilters() {
      ...
      const today = new Date()
      const fmt = d => `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`
      this.filters = { start: fmt(yesterday), end: fmt(today), 锁类分区: '', 序列号: '', 姓名: '', 工单批号: '', 含TSA: true }
    ...
    async exportData() {
      ...
      this.exporting = true
      try {
        const params = { start: this.filters.start, end: this.filters.end, 含TSA: this.filters.含TSA }
        if (this.filters.序列号) params.序列号 = this.filters.序列号.trim()
        if (this.filters.姓名) params.姓名 = this.filters.姓名.trim()
        if (this.filters.工单批号) params.工单批号 = this.filters.工单批号.trim()
        if (this.filters.锁类分区) params.锁类分区 = this.filters.锁类分区.trim()
        const res = await axios.get('/api/workshopReportDetail/Zhuangqian/export', { params, responseType: 'blob' })
        const cd = res.headers['content-disposition']; let fname = '装嵌报工详情.xlsx'; if (cd) { const m = cd.match(/filename\*=UTF-8''(.+)/); if (m) fname = decodeURIComponent(m[1]); else { const p = cd.split('filename=')[1]; if (p) fname = p.replace(/"/g, '') } }
