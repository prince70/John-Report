...
export default {
  ...
  data() {
    return {
      breadcrumbItems: ['报表页面', '全流程报工库存-电子锁车间'],
  ...
  methods: {
    generateBreadcrumb(path) {
      ...
      } catch { this.breadcrumbItems = ['报表页面', '全流程报工库存-电子锁车间'] }
    },
    ...
    async loadOptions() {
      try {
        ...
        const results = await Promise.all(fields.map(f => axios.get('/api/fullProcessInventoryDZS/options', { params: { field: f } }).then(res => ({ f, d: res.data })).catch(() => ({ f, d: { data: [] } }))))
    ...
    async searchData() {
      ...
      try {
        ...
        const [dataRes, summaryRes] = await Promise.all([
          axios.get('/api/fullProcessInventoryDZS', { params }),
          axios.get('/api/fullProcessInventoryDZS/summary', { params })
        ])
        ...
        if (summaryRes.data?.status === 'success') {
          const order = ['PCBA检测','锁体壳组装','马达检测','焊马达','Yardo终检-包装','底盖套圈','气密性-镭射','焊正极片','焊负极片','锁芯壳组装1','锁芯壳组装2','贴装饰圈','运行测试','整锁组装','组装四件套']
