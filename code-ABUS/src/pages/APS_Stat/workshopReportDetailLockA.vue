...
export default {
  ...
  data() {
    ...
    return {
      breadcrumbItems: ['报工', '车间报工详情', '锁体A报工详情'],
  ...
  methods: {
    generateBreadcrumb() { this.breadcrumbItems = ['报工', '车间报工详情', '锁体A报工详情'] },
    ...
    async searchData() {
      ...
      try {
        ...
        const res = await axios.get('/api/workshopReportDetail/LockA', { params })
    ...
    async exportData() {
      ...
      try {
        ...
        if (this.filters.姓名) params.姓名 = this.filters.姓名.trim()
        if (this.filters.订单批号) params.订单批号 = this.filters.订单批号.trim()
        const res = await axios.get('/api/workshopReportDetail/LockA/export', { params, responseType: 'blob' })
        const cd = res.headers['content-disposition']; let fname = '锁体A报工详情.xlsx'; if (cd) { const m = cd.match(/filename\*=UTF-8''(.+)/); if (m) fname = decodeURIComponent(m[1]); else { const p = cd.split('filename=')[1]; if (p) fname = p.replace(/"/g, '') } }
