...
export default {
  ...
  data() {
    return {
      breadcrumbItems: ['报表页面', '全流程报工库存-装嵌车间-功能锁区'],
  ...
  methods: {
    generateBreadcrumb(path) {
      ...
      } catch { this.breadcrumbItems = ['报表页面', '全流程报工库存-装嵌车间-功能锁区'] }
    },
    ...
    async loadOptions() {
      try {
        ...
        const results = await Promise.all(fields.map(f => axios.get('/api/fullProcessInventoryZQ_GNSQ/options', { params: { field: f } }).then(res => ({ f, d: res.data })).catch(() => ({ f, d: { data: [] } }))))
    ...
    async searchData() {
      ...
      try {
        ...
        const [dataRes, summaryRes] = await Promise.all([
          axios.get('/api/fullProcessInventoryZQ_GNSQ', { params }),
          axios.get('/api/fullProcessInventoryZQ_GNSQ/summary', { params })
        ])
        ...
        if (summaryRes.data?.status === 'success') {
          ...
          this.summaryData = list.sort((a, b) => {
            const ia = a.当前工序ID; const ib = b.当前工序ID
            return ia - ib
          })
        }
    ...
    resetFilters() {
      this.filters = { 订单批号: '', 料品编码: '', part_name: '', part_spec: '', 生产车间: '', 当前工序ID: '', 下一道工序ID: '', 开始日期: '', 结束日期: '' }
      this.searchData()
    }
  }
}
